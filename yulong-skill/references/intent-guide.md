...
当 CLI 以 `--token` 模式部署时，认证由部署环境/模型上下文管理：

- 用户提到"登录/token/认证/重新登录" → 告知"当前为 Token 模式，认证由部署环境管理，请勿执行 `auth login`"
- 用户提到"退出/登出" → 告知"Token 模式下无需登出，token 由上游注入"
- 用户提到"刷新权限/更新权限" → 本地模式使用 `auth refresh-permissions`；Token 模式下由 CLI 自动在每次启动时拉取，无需显式刷新
