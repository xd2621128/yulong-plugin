...
### `project business list` / `project origin-contract forward list` / `project edaLabel beforeSplit/afterSplit` / `project partner page`
...
| 命令 | 常见错误 | 恢复动作 |
|---|---|---|
| `project crmField dept` | 返回超长列表 | 使用 `--fields` 或本地 `grep`/`jq` 过滤 |
| `project crmField region` | `regionId` 找不到 | 确认使用 `id` 字段而非 `name` |
| `project crmField province` | `areaId` 无效 | 确认使用叶子节点的 `id`，而非父级或 `value` 名称 |
| `project system pm-index listRDField` | 返回为空 | 确认接口已开放，或检查请求参数 |
