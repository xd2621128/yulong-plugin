...
| `incomeTimeBegin` | string | 收入月份开始（YYYY-MM） | `"2025-12"` |
| `incomeTimeEnd` | string | 收入月份结束（YYYY-MM） | `"2025-12"` |
...
2. **月份字段是 `incomeTimeBegin/End`**：格式为 `YYYY-MM`，不是 `archiveTime`；前端月份选择器用的是 `incomeTime`
...
| "按时间查收入清单" / "XX 年 XX 月的收入清单" | `project edaLabel beforeSplit/afterSplit` + `incomeTimeBegin/End` | 按收入月份筛选 |
...
yulong project edaLabel beforeSplit --format json --json '{
  ...
  "incomeTimeBegin": "2025-12",
  "incomeTimeEnd": "2025-12"
}'
```
